...
export default function AuditorUnavailabilityForm() {
  ...
  const [annualLeave, setAnnualLeave] = useState<boolean>(false);
  const [unpaidLeave, setUnpaidLeave] = useState<boolean>(false);
  const [otherLeave, setOtherLeave] = useState<boolean>(false);
  const [otherLeaveDetails, setOtherLeaveDetails] = useState<string>("");

  // Mission fields
  const [missionDepartureTime, setMissionDepartureTime] = useState<string>("");
  const [missionReturnTime, setMissionReturnTime] = useState<string>("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!demandType) {
      setErrorMsg("Please select a demand type.");
      return;
    }
    ...
    try {
      ...
      // Add conditional fields based on demand type
      if (demandType === "Authorization") {
        requestBody.departure_time = departureTime;
        requestBody.return_time = returnTime;
      } else if (demandType === "Leave") {
        requestBody.leave_details = {
          half_day: halfDay,
          annual_leave: annualLeave,
          unpaid_leave: unpaidLeave,
          other: otherLeave,
          other_details: otherLeave ? otherLeaveDetails : null,
        };
      } else if (demandType === "Mission") {
        requestBody.mission_departure_time = missionDepartureTime;
        requestBody.mission_return_time = missionReturnTime;
        requestBody.travel_expenses = travelExpenses;
      }
      ...
      setMissionDepartureTime("");
      setMissionReturnTime("");
      setTravelExpenses("");
    } catch (err: any) {
            ...
            <form onSubmit={handleSubmit} className="px-8 py-8 space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Type of Demand <span className="text-red-500">*</span>
                </label>
                <Select
                  options={[
              ...
              {/* Conditional Fields for Authorization */}
              {demandType === "Authorization" && (
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center space-x-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={unpaidLeave}
                        onChange={(e) => setUnpaidLeave(e.target.checked)}
                        className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                      />
                      <span className="text-sm font-medium text-gray-700">
                        Unpaid Leave
                    ...
                    <label className="flex items-center space-x-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={otherLeave}
                        onChange={(e) => setOtherLeave(e.target.checked)}
                        className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                      />
                      <span className="text-sm font-medium text-gray-700">
              ...
              {/* Conditional Fields for Mission */}
              {demandType === "Mission" && (
                <div className="space-y-4 bg-orange-50 border border-orange-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        type="time"
                        value={missionDepartureTime}
                        onChange={(e) =>
                          setMissionDepartureTime(e.target.value)
                        }
                        className="w-full rounded-lg border-2 border-gray-200 bg-white px-4 py-3 text-gray-900 outline-none transition-all focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                        required={true}
                      />
                    ...
                    <div>
                      ...
                      <Input
                        type="time"
                        value={missionReturnTime}
                        onChange={(e) => setMissionReturnTime(e.target.value)}
                        className="w-full rounded-lg border-2 border-gray-200 bg-white px-4 py-3 text-gray-900 outline-none transition-all focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                        required={true}
                      />
                    </div>
                  ...
                  <div>
                    <Label className="block text-sm font-medium text-gray-700 mb-2">
                      Travel Expenses <span className="text-red-500">*</span>
                    </Label>
                    <Input
                      type="text"
                      value={travelExpenses}
                      onChange={(e) => setTravelExpenses(e.target.value)}
                      placeholder="Enter travel expenses amount"
                      className="w-full rounded-lg border-2 border-gray-200 bg-white px-4 py-3 text-gray-900 outline-none transition-all focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                      required={true}
                    />
                  </div>
